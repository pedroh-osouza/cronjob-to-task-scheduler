...
class Task {
    ...
    async toSchedule(scheduleXmlObject) {
    ...
    existsTask(taskName) {
        (0, child_process_1.exec)(`schtasks /query /TN "${taskName}"`, (error, stdout, stderr) => {
            if (!error)
                throw new DuplicatedTaskException_1.DuplicatedTaskException(this.taskName);
        });
    }
}
exports.Task = Task;
