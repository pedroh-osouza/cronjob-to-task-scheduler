...
const XmlException_1 = require("../Exceptions/XmlException");
const SchtasksCommandException_1 = require("../Exceptions/SchtasksCommandException");
class Task {
    ...
    toSchedule(scheduleXmlObject) {
        ...
        fs_1.default.writeFile(xmlFilePath, xml, (err => {
            if (err)
                throw new XmlException_1.XmlException('error when creating task xml');
            const command = `schtasks /create /tn "${this.taskName}" /xml "${xmlFilePath}"`;
            (0, child_process_1.exec)(command, (error, stdout, stderr) => {
                if (error)
                    throw new SchtasksCommandException_1.SchtasksCommandException('error when running schedule command');
